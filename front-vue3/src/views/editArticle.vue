    ...
    >
      ...
      <div class="article-post">
        <el-card :body-style="{ padding: '5px' }">
          ...
          <!--内容-->
          <!--label-width设置为0为了铺满屏幕-->
          <el-form-item prop="content" label-width="0">
            <ArticleMarkdown
              v-model="articleDetail.markdownContent"
      ...
      <div class="article-setting">
        <el-card>
          ...
          <el-form-item label="分类" prop="categoryId">
            <el-select
              v-model="articleDetail.categoryId"
              placeholder="请选择分类"
              style="width: 100%"
            >
              <el-option
                v-for="item in bCategoryList"
                :key="item.id"
          ...
          <el-form-item label="标签" prop="tagId">
            <el-select
              v-model="articleDetail.tagId"
              placeholder="请选择标签"
              style="width: 100%"
            >
              <el-option
                v-for="item in bTagsList"
                :key="item.id"
                :label="item.tagName"
                :value="item.id"
              ></el-option>
          ...
          <!--保存按钮-->
          <el-form-item>
            <el-button type="primary" style="width: 100%" @click="saveArticle"
              >保存</el-button
            >
          </el-form-item>
        </el-card>
...
const getBArticleById = async () => {
  let articleId = route.params.articleId;
  const res = await proxy.$api.queryBArticleById(articleId);
  if (res.code != 200) {
    proxy.$message.error(res.message);
  } else {
    articleDetail.value = res.data;
    // console.log("文章详情：", artcileDetail.value.id)
  }
};
getBArticleById();
...
const handleAvatarSuccess = (res, file) => {
  articleDetail.value.image = res.data;
};

//设置markdown编辑器的富文本信息
const setHtmlContent = (htmlContent) => {
  articleDetail.value.content = htmlContent;
};
...
const saveArticle = () => {
  proxy.$refs["formDataRef"].validate((valid) => {
    if (valid) {
      proxy.$api.updateBArticle(articleDetail.value).then((res) => {
        if (res.code != 200) {
          proxy.$message.error(res.message);
        } else {
