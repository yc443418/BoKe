<template>
    <div class="body-container">
        <div class="category">
            <!--头部分类数量-->
            <div class="header">
                <span>共计{{ bCategoryList.length - 1 }}分类</span>
            </div>
            <!--左侧分类列表-->
            <div class="category-panel">
                <el-tabs v-model="activeName" tab-position="left" @tab-click="handleTabClick">
                    <el-tab-pane v-for="(item, index) in bCategoryList" :name="index + ''" :key="index">
                        <template #label>
            ...
            <!--右侧分类列表-->
            <div class="category-article" v-for="item in articleList" :key="item.id">
...
const getBCategoryList = async () => {
    ...
    if (res.code !== 200) {
        ...
        bCategoryList.value.push(...res.data)
    }
}
getBCategoryList()
...
const handleTabClick = async (tab) => {
    let item = bCategoryList.value[tab.index]
