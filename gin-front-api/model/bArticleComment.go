...
type BArticleComment struct {
	...
	Children          []BArticleComment `json:"children" gorm:"-"`                                                                  // 子集
}
...
// AddCommentDto 新增评论Dto
type AddCommentDto struct {
	ArticleId   uint   `json:"articleId" validate:"required"` // 文章ID
	PCommentId  uint   `json:"pCommentId"`                    // 父级评论ID
	Content     string `json:"content" validate:"required"`   // 回复内容
	ReplyUserId uint   `json:"replyUserId"`                   // 回复人的id
}

// CommentTopDto 评论置顶
type CommentTopDto struct {
	Id      uint `json:"id"`      // 评论ID
	TopType uint `json:"topType"` // 1:未置顶 2:置顶
}
